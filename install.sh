...
if [ "$(lsb_release -rs)" = "8*|9*|10*|11*|16.04*|18.04*" ]; then
 ...
 # [+get files ⇣⇣⇣+]
  ...
  wget "https://raw.githubusercontent.com/tryoo127/udp-custom/main/bin/udp-custom-linux-amd64" -O /root/udp/udp-custom &>/dev/null
  # wget "https://raw.githubusercontent.com/tryoo127/udp-custom/main/bin/udp-request-linux-amd64" -O /usr/bin/udp-request &>/dev/null
  ...
  # [+menu+]
  wget -O /usr/bin/udp 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/udp' 
  chmod +x /usr/bin/udp
  ...
  print_center -ama "${a103:-  Server Will Reboot: \nudp\n}"
  msg -bar
  time_reboot 5
fi
