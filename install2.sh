...
source <(curl -sSL 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/module')

time_reboot() {
  print_center -ama "${a92:-Server Will Reboot In} $1 ${a93:-Seconds}"
  REBOOT_TIMEOUT="$1"
...
if [ "$(lsb_release -rs)" = "8*|9*|10*|11*|16.04*|18.04*" ]; then
  ...
  clear
  echo ""
  print_center -ama " ⇢ UDP Custom Installation start... <"
  sleep 3
 ...
 # [+get files ⇣⇣⇣+]
  source <(curl -sSL 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/module') &>/dev/null
  wget -O /etc/UDPCustom/module 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/module' &>/dev/null
  chmod +x /etc/UDPCustom/module

  wget "https://raw.githubusercontent.com/tryoo127/udp-custom/main/udp-custom-linux-amd64" -O /root/udp/udp-custom &>/dev/null
  # wget "https://raw.githubusercontent.com/tryoo127/udp-custom/main/udp-request-linux-amd64" -O /usr/bin/udp-request &>/dev/null
  chmod +x /root/udp/udp-custom
  # chmod +x /usr/bin/udp-request

  wget -O /etc/limiter.sh 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/limiter.sh'
  cp /etc/limiter.sh /etc/UDPCustom
  chmod +x /etc/limiter.sh
  chmod +x /etc/UDPCustom
  
  # [+udpgw+]
  wget -O /etc/udpgw 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/udpgw'
  mv /etc/udpgw /bin
  chmod +x /bin/udpgw

  # [+service+]
  wget -O /etc/udpgw.service 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/udpgw.service'
  wget -O /etc/udp-custom.service 'https://raw.githubusercontent.com/tryoo127/udp-custom/main/udp-custom.service'
  ...
  # [+menu+]
  wget -O /usr/bin/udp 'https://raw.githubusercontent.com/NevermoreSSH/SSHUDPCUSTOM/main/module/udp' 
  chmod +x /usr/bin/udp
